...
export default function Dashboard() {
  ...
  // 1. IDENTIDADE E NAVEGAÇÃO (Correção: Iniciando vazio para não repetir seu nome)
  ...
  // 4. CARREGAMENTO DE DADOS (Join Técnico)
  ...
  // 5. AÇÕES TÉCNICAS (CSV + COORDENADOR LIBERADO)
  async function handleSalvar() {
    if (userCargo !== 'Diretor' && userCargo !== 'Coordenador') {
      return alert('Acesso Negado: Função restrita à liderança técnica.')
    }
  ...
  function exportarCSV() {
    ...
    const csv = demandas.map(d => {
      const resp = Array.isArray(d.perfis) ? d.perfis[0]?.nome_completo : d.perfis?.nome_completo
      return `${d.num_processo},${d.cliente},${d.titulo},${resp || 'N/A'},${d.vencimento},${d.status}`
    }).join("\n")
    ...
    <div className="flex min-h-screen bg-[#0a0c10] text-[#d1d5db] font-sans uppercase overflow-hidden">
      ...
      <div className="w-64 bg-[#161b22] border-r border-[#30363d] flex flex-col p-6 gap-8 shadow-2xl z-20">
        ...
        <div className="mt-auto border-t border-[#30363d] pt-6">
          <p className="text-[10px] text-white font-black truncate">{userName || 'USUÁRIO'}</p>
      ...
      <div className="flex-1 overflow-y-auto p-8 relative">
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-6 bg-[#161b22] p-4 border border-[#30363d] rounded-lg shadow-xl">
              ...
              {(userCargo === 'Diretor' || userCargo === 'Coordenador') && (
                <button onClick={() => { setEditId(null); limparCampos(); setIsModalOpen(true); }} className="bg-[#00c58e] text-black px-6 py-3 rounded font-black text-[10px] shadow-lg">+ ADICIONAR DEMANDA</button>
              )}
            </div>

            <div className="flex gap-2 mb-4">
              <input placeholder="FILTRAR..." className="flex-1 bg-[#0d1117] border border-[#30363d] p-4 rounded text-[11px] text-white font-bold outline-none focus:border-emerald-500" value={filtroTexto} onChange={e => setFiltroTexto(e.target.value)} />
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-[11px] font-bold">
                  {filtradas.map((item) => {
                    ...
                    // Correção de Identidade na Tabela: Pega o nome do Join
                    const respNome = Array.isArray(item.perfis) ? item.perfis[0]?.nome_completo : item.perfis?.nome_completo
                    ...
                    return (
                      <React.Fragment key={item.id}>
                        <tr onClick={() => setExpandedRowId(isExpanded ? null : item.id)} className={`border-b border-[#30363d]/50 transition-all cursor-pointer ${getRowColor(item)}`}>
                          ...
                          <td className="p-4"><span className="bg-black/20 px-2 py-1 rounded text-[9px] font-black">{respNome || 'NÃO ATRIBUÍDO'}</span></td>
        ...
        {/* ABA OPERAÇÃO */}
        {activeTab === 'clientes' && (
          <div className="max-w-7xl mx-auto"><h2 className="text-2xl font-black italic text-emerald-500 mb-8 border-b border-[#30363d] pb-4 uppercase">Demandas Operação Campo</h2></div>
        )}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-[#161b22] p-8 rounded border border-[#30363d] w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
              <h2 className="text-emerald-500 font-black italic mb-8 border-b border-[#30363d] pb-2 uppercase tracking-widest text-lg italic">Gestão Técnica</h2>
