...
export default function Dashboard() {
  ...
  async function handleCriarPerfil() {
    ...
    await supabase.from('perfis').insert([{ id: userId, nome_completo: nomeOnboarding, cargo: cargoOnboarding, empresa: 'Ecominas Mineração' }])
    setIsFirstLogin(false); carregarDados();
  }
  ...
  async function handleExcluir(id: string) {
    if (userCargo !== 'Diretor') return alert('Acesso restrito.')
  ...
  const getRowColor = (item: any) => {
    ...
    if (item.vencimento && item.vencimento < hojeStr) return 'bg-[#da3633]/20 text-[#f85149] border-l-4 border-red-600'
                    ...
                    <React.Fragment key={item.id}>
                      <tr onClick={() => setExpandedId(expandedId === item.id ? null : item.id)} className={`border-b border-[#30363d]/50 cursor-pointer transition-all ${getRowColor(item)}`}>
                        ...
                        <td className="p-4 text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex justify-end gap-3 items-center italic font-black">
                            ...
                            {(userCargo === 'Diretor' || userCargo === 'Coordenador') && <button onClick={() => { setEditId(item.id); setIsModalOpen(true); }} className="p-1 text-slate-400 italic">✎</button>}
                            {userCargo === 'Diretor' && <button onClick={() => handleExcluir(item.id)} className="text-red-500">🗑️</button>}
                          </div>
                        </td>
                      </tr>
                      {expandedId === item.id && (
                        <tr className="bg-[#0d1117] border-b border-[#30363d] animate-in slide-in-from-top-1">
                          <td colSpan={6} className="p-6">
                            <div className="grid grid-cols-2 gap-8 uppercase">
                              ...
                              <div><h4 className="text-emerald-500 text-[9px] font-black mb-2 italic">Notas Técnicas Operacionais</h4><p className="text-slate-300 leading-relaxed font-bold">{item.descricao || 'Sem observações cadastradas.'}</p></div>
                            </div>
                          </td>
                        </tr>
                      )}
              ...
              <table className="w-full text-left border-collapse text-[11px] font-bold uppercase">
                <thead>
                  <tr className="bg-[#0d1117] text-[10px] font-black text-slate-500 border-b border-[#30363d]">
                    ...
                    <th className="p-4 text-center italic">Equipe Presente</th>
                  </tr>
                </thead>
                <tbody>
                  {viagens.map((v) => (
                    <React.Fragment key={v.id}>
                      <tr onClick={() => setExpandedId(expandedId === v.id ? null : v.id)} className="border-b border-[#30363d]/50 hover:bg-[#1c2128] cursor-pointer transition-all">
                        ...
                        <td className="p-4 font-black text-emerald-500 bg-emerald-900/5 text-center">{v.km_final - v.km_inicial} KM</td>
                        <td className="p-4 font-black text-blue-400 text-center">R$ {(Number(v.custo_combustivel) + Number(v.custo_alimentacao) + Number(v.custo_hospedagem) + Number(v.custo_pedagio || 0) + Number(v.custo_outros || 0)).toFixed(2)}</td>
                        <td className="p-4 text-[9px] text-center text-slate-400 italic">{v.participantes || 'SOLO'}</td>
                      </tr>
                      {expandedId === v.id && (
                        <tr className="bg-[#0d1117] border-b border-[#30363d] animate-in slide-in-from-top-1">
                          <td colSpan={4} className="p-6">
                            ...
                            <div className="bg-[#161b22] p-4 rounded border border-[#30363d] uppercase italic"><h4 className="text-emerald-500 text-[9px] font-black mb-1">Motivo / Jazida Alvo / Observações de Rota</h4><p className="text-slate-300 font-bold">{v.descricao || '--'}</p></div>
                          </td>
                        </tr>
                      )}
                    </React.Fragment>
        ...
        {isModalViagemOpen && (
          <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 backdrop-blur-sm uppercase">
            <div className="bg-[#161b22] p-8 rounded border border-[#30363d] w-full max-w-2xl shadow-2xl overflow-y-auto uppercase">
              <h2 className="text-emerald-500 font-black italic mb-8 border-b border-[#30363d] pb-2 text-lg uppercase tracking-widest text-left italic">Registro de Viagem</h2>
              <div className="space-y-6 text-left font-black">
                <div><label className="text-[8px] text-slate-500 block mb-1 italic">Motorista *</label><select className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white font-bold outline-none focus:border-emerald-500 uppercase" value={responsavelViagemId} onChange={e => setResponsavelViagemId(e.target.value)}><option value="">SELECIONE UM PROFISSIONAL</option>{equipe.map(tec => (<option key={tec.id} value={tec.id}>{tec.nome_completo} ({tec.cargo})</option>))}</select></div>
                <div><label className="text-[8px] text-slate-500 block mb-2 italic">Equipe</label><div className="flex flex-wrap gap-2 p-3 bg-[#0d1117] border border-[#30363d] rounded">{equipe.map(tec => (<button key={tec.id} onClick={() => toggleParticipante(tec.nome_completo)} className={`px-3 py-2 rounded text-[9px] font-black transition-all ${participantesSelecionados.includes(tec.nome_completo) ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/40' : 'bg-slate-800 text-slate-500'}`}>{participantesSelecionados.includes(tec.nome_completo) ? '✓ ' : '+ '} {tec.nome_completo}</button>))}</div></div>
                <div className="grid grid-cols-2 gap-4 uppercase italic">
                  <div><label className="text-[8px] text-slate-500 block mb-1">Km Inicial</label><input type="number" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white focus:border-emerald-500 outline-none font-mono" value={kmInicial} onChange={e => setKmInicial(Number(e.target.value))} /></div>
                  <div><label className="text-[8px] text-slate-500 block mb-1">Km Final</label><input type="number" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white focus:border-emerald-500 outline-none font-mono" value={kmFinal} onChange={e => setKmFinal(Number(e.target.value))} /></div>
                </div>
                <div className="bg-emerald-900/10 p-4 rounded border border-emerald-500/20 text-center font-black italic"><p className="text-[10px] text-emerald-500 uppercase tracking-widest font-mono">Distância: {kmFinal - kmInicial} KM</p></div>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-left uppercase italic font-black">
                  <div><label className="text-[8px] text-slate-500 block mb-1">Combustível</label><input type="number" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white focus:border-emerald-500 font-mono outline-none" value={fuel} onChange={e => setFuel(Number(e.target.value))} /></div>
                  <div><label className="text-[8px] text-slate-500 block mb-1">Alimentação</label><input type="number" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white focus:border-emerald-500 font-mono outline-none" value={food} onChange={e => setFood(Number(e.target.value))} /></div>
                  <div><label className="text-[8px] text-slate-500 block mb-1">Hospedagem</label><input type="number" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white focus:border-emerald-500 font-mono outline-none" value={hotel} onChange={e => setHotel(Number(e.target.value))} /></div>
                  <div><label className="text-[8px] text-emerald-500 block mb-1 tracking-tighter">Pedágio</label><input type="number" className="w-full bg-[#0d1117] border border-emerald-500/30 p-3 text-xs text-white focus:border-emerald-500 font-mono outline-none" value={toll} onChange={e => setToll(Number(e.target.value))} /></div>
                  <div><label className="text-[8px] text-blue-500 block mb-1 tracking-tighter">Outros</label><input type="number" className="w-full bg-[#0d1117] border border-blue-500/30 p-3 text-xs text-white focus:border-blue-500 font-mono outline-none" value={others} onChange={e => setOthers(Number(e.target.value))} /></div>
                </div>
                <div><label className="text-[8px] text-slate-500 block mb-1 uppercase tracking-widest italic">Motivo / Jazida / Notas</label><textarea className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white h-24 resize-none focus:border-emerald-500 uppercase shadow-inner" value={descricaoViagem} onChange={e => setDescricaoViagem(e.target.value)} /></div>
              </div>
              <div className="flex gap-4 mt-10"><button onClick={handleSalvarViagem} className="flex-1 bg-emerald-600 p-4 font-black uppercase text-xs text-black shadow-lg hover:bg-emerald-500 transition-all active:scale-95 italic">Salvar Registro</button><button onClick={() => setIsModalViagemOpen(false)} className="flex-1 bg-slate-800 p-4 font-black uppercase text-xs text-slate-400 uppercase tracking-widest italic">Cancelar</button></div>
            </div>
          </div>
        )}
            ...
            <div className="bg-[#161b22] p-8 rounded border border-[#30363d] w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto uppercase text-left">
              ...
              <div className="space-y-4 font-black italic">
                 ...
                 <div className="grid grid-cols-2 gap-3 uppercase">
                   <div><label className="text-[8px] text-slate-500 block mb-1 uppercase">Processo ANM</label><input className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white outline-none focus:border-emerald-500 shadow-inner" value={numProcesso} onChange={e => setNumProcesso(e.target.value)} /></div>
                   <div><label className="text-[8px] text-slate-500 block mb-1 uppercase">Cliente</label><input className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white outline-none focus:border-emerald-500 shadow-inner" value={cliente} onChange={e => setCliente(e.target.value)} /></div>
                 </div>
                 <div><label className="text-[8px] text-slate-500 block mb-1 uppercase italic tracking-widest">Título da Demanda</label><input className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white font-bold outline-none focus:border-emerald-500 shadow-inner uppercase" value={novoTitulo} onChange={e => setNovoTitulo(e.target.value)} /></div>
                 <div className="grid grid-cols-2 gap-3 uppercase">
                   <div><label className="text-[8px] text-slate-500 block mb-1 italic">Vencimento ANM</label><input type="date" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white outline-none focus:border-emerald-500" value={vencimento} onChange={e => setVencimento(e.target.value)} /></div>
                   <div><label className="text-[8px] text-slate-500 block mb-1 italic">Criticidade</label><select className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white font-bold outline-none focus:border-emerald-500" value={ranking} onChange={e => setRanking(e.target.value)}><option value="2">NORMAL</option><option value="3">URGENTE (!)</option><option value="4">CRÍTICA (!!!)</option></select></div>
                 </div>
                 <div><label className="text-[8px] text-emerald-500 block mb-1 uppercase italic tracking-widest text-left">Link SEI / ANM Digital</label><input placeholder="LINK DO PROCESSO" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white outline-none focus:border-emerald-500 uppercase font-mono shadow-inner" value={linkProjeto} onChange={e => setLinkProjeto(e.target.value)} /></div>
                 <div><label className="text-[8px] text-emerald-500 block mb-1 uppercase italic tracking-widest text-left">Notas Técnicas</label><textarea placeholder="DESCREVA OS DETALHES TÉCNICOS" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white h-24 uppercase outline-none focus:border-emerald-500 resize-none shadow-inner italic" value={novaDescricao} onChange={e => setNovaDescricao(e.target.value)} /></div>
              </div>
              <div className="flex gap-4 mt-10"><button onClick={handleSalvar} className="flex-1 bg-emerald-600 p-4 font-black uppercase text-xs text-black shadow-lg hover:bg-emerald-500 transition-all active:scale-95 italic">Salvar Registro</button><button onClick={() => setIsModalOpen(false)} className="flex-1 bg-slate-800 p-4 font-black uppercase text-xs text-slate-400 uppercase tracking-widest italic">Fechar</button></div>
            </div>
          </div>
        )}
