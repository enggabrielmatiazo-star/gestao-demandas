...
export default function Dashboard() {
  ...
  // 3. ESTADOS DO SISTEMA (VIAGENS) - NOVA PARTE
  ...
  // 5. ESTADOS DO FORMULÁRIO (MODAL DEMANDAS)
  ...
  function limparCamposViagem() {
    setKmInicial(0); setKmFinal(0); setFuel(0); setFood(0); setHotel(0);
    setParticipantes(''); setDescricaoViagem('');
  }

  // AÇÕES DE GESTÃO (DEMANDAS)
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0c10] text-[#d1d5db] font-sans uppercase overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-8 relative">
        ...
        {/* ABA 2: OPERAÇÃO (CRIADA ANTERIORMENTE) */}
        {activeTab === 'clientes' && (
          <div className="max-w-7xl mx-auto"><h2 className="text-2xl font-black italic text-emerald-500 mb-8 border-b border-[#30363d] pb-4">Demandas de Operação</h2></div>
        )}

        {/* ABA 3: CONTROLE DE VIAGENS [NOVO MÓDULO] */}
        {activeTab === 'viagens' && (
          <div className="max-w-7xl mx-auto">
                ...
                <tbody className="text-[11px] font-bold">
                  {viagens.map((v) => (
                    <tr key={v.id} className="border-b border-[#30363d]/50 hover:bg-[#1c2128]">
                      <td className="p-4">
                        <p className="uppercase">{v.perfis?.nome_completo || 'CARGA'}</p>
                        <p className="text-[9px] text-slate-500">{new Date(v.created_at).toLocaleDateString('pt-BR')}</p>
                      </td>
                      <td className="p-4 font-mono">{v.km_inicial}</td>
                      <td className="p-4 font-mono">{v.km_final}</td>
                      ...
                      <td className="p-4 text-slate-400 italic">{v.participantes || '--'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-[#161b22] p-8 rounded border border-[#30363d] w-full max-w-2xl shadow-2xl overflow-y-auto uppercase">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                 ...
                 <div className="md:col-span-2 bg-emerald-900/10 p-3 rounded border border-emerald-500/20 text-center"><p className="text-[10px] font-black text-emerald-500">RODAGEM TOTAL: {kmFinal - kmInicial} KM</p></div>
                 ...
                 <div><label className="text-[8px] font-black text-slate-500 block mb-1">PARTICIPANTES</label><input placeholder="NOMES DA EQUIPE" className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white" value={participantes} onChange={e => setParticipantes(e.target.value)} /></div>
                 <div className="md:col-span-2"><label className="text-[8px] font-black text-slate-500 block mb-1">MOTIVO / ROTA</label><textarea className="w-full bg-[#0d1117] border border-[#30363d] p-3 text-xs text-white h-20 resize-none" value={descricaoViagem} onChange={e => setDescricaoViagem(e.target.value)} /></div>
              </div>
              <div className="flex gap-4 mt-10"><button onClick={handleSalvarViagem} className="flex-1 bg-emerald-600 p-4 font-black uppercase text-xs text-black shadow-lg hover:bg-emerald-500 transition-all">SALVAR VIAGEM</button><button onClick={() => setIsModalViagemOpen(false)} className="flex-1 bg-slate-800 p-4 font-black uppercase text-xs text-slate-400">CANCELAR</button></div>
            </div>
          </div>
        )}
        ...
        {/* MODAL GESTÃO DEMANDAS (ORIGINAL PRESERVADO) */}
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
