...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState('demandas') // Controla qual página aparece
  const [userId, setUserId] = useState<string | null>(null)
  const [userName, setUserName] = useState('Gabriel Matiazo') 
  const [userCargo, setUserCargo] = useState('')

  // 2. ESTADOS DO SISTEMA (Demandas e Equipe)
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0c10] text-[#d1d5db] font-sans uppercase overflow-hidden">
      ...
      <div className="w-64 bg-[#161b22] border-r border-[#30363d] flex flex-col p-6 gap-8 shadow-2xl z-20">
        <div className="bg-emerald-600 px-4 py-3 font-black text-white italic text-2xl rounded text-center shadow-lg shadow-emerald-900/20">ECOMINAS</div>
        
        <nav className="flex flex-col gap-2">
          <button 
            onClick={() => setActiveTab('demandas')}
            className={`flex items-center gap-3 p-4 rounded font-black text-[11px] transition-all ${activeTab === 'demandas' ? 'bg-emerald-600/10 text-emerald-500 border border-emerald-500/30' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
          >
            📋 CENTRAL DE DEMANDAS
          </button>
          <button 
            onClick={() => setActiveTab('clientes')}
            className={`flex items-center gap-3 p-4 rounded font-black text-[11px] transition-all ${activeTab === 'clientes' ? 'bg-emerald-600/10 text-emerald-500 border border-emerald-500/30' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
          >
            🤝 CLIENTES FIXOS
          </button>
          <button 
            className="flex items-center gap-3 p-4 rounded font-black text-[11px] text-slate-700 cursor-not-allowed italic"
            disabled
          >
        ...
        <div className="mt-auto border-t border-[#30363d] pt-6">
          <p className="text-[9px] text-slate-500 font-bold uppercase mb-1">Responsável</p>
      ...
      <div className="flex-1 overflow-y-auto p-8 relative">
        ...
        {/* ABA: CENTRAL DE DEMANDAS (O layout que você já gosta) */}
        {activeTab === 'demandas' && (
          <div className="max-w-7xl mx-auto animate-in fade-in duration-500">
            {/* HEADER COM FILTROS STATUS */}
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 bg-[#161b22] p-4 border border-[#30363d] rounded-lg gap-4 shadow-xl">
              <div className="text-[11px] font-black tracking-widest text-white italic">VISÃO GERAL OPERACIONAL</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-[#161b22] p-6 border border-[#30363d] rounded-lg">
                <h3 className="text-slate-500 text-[9px] font-black tracking-widest uppercase">Em Aberto</h3>
                <p className="text-4xl font-black text-emerald-500 mt-1">{c_abertas}</p>
              </div>
              <div className="bg-[#161b22] p-6 border border-[#30363d] rounded-lg">
                <h3 className="text-slate-500 text-[9px] font-black tracking-widest uppercase">Concluídas</h3>
              ...
              <div className="bg-[#161b22] p-6 border border-red-900/30 rounded-lg animate-pulse">
                <h3 className="text-red-500/50 text-[9px] font-black tracking-widest uppercase">Atrasadas</h3>
            ...
            <div className="flex gap-2 mb-4">
              <input placeholder="FILTRAR POR PROCESSO, TÍTULO OU CLIENTE..." className="flex-1 bg-[#0d1117] border border-[#30363d] p-4 rounded text-[11px] text-white font-bold outline-none focus:border-emerald-500 transition-all shadow-inner" value={filtroTexto} onChange={e => setFiltroTexto(e.target.value)} />
                            ...
                            <td colSpan={6} className="p-6">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in slide-in-from-top-2">
                                <div>
                                  ...
                                  {item.link_projeto ? (
                                    <a href={item.link_projeto} target="_blank" rel="noreferrer" className="text-white hover:text-emerald-400 text-[11px] underline break-all font-mono italic">{item.link_projeto}</a>
        ...
        {/* ABA: CLIENTES FIXOS (Nova Página Exploratória) */}
        {activeTab === 'clientes' && (
          <div className="max-w-7xl mx-auto animate-in slide-in-from-right duration-500">
            <h2 className="text-2xl font-black italic text-emerald-500 mb-8 border-b border-[#30363d] pb-4 tracking-widest uppercase">Portfólio de Clientes Fixos</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Exemplo de Card de Cliente */}
              <div className="bg-[#161b22] p-8 border border-[#30363d] rounded-lg hover:border-emerald-500/50 transition-all">
                <div className="bg-emerald-500/10 text-emerald-400 text-[10px] font-black px-3 py-1 rounded w-fit mb-4">CONTRATO ATIVO</div>
                <h3 className="text-lg font-black text-white mb-2 uppercase">Mineradora Calforte</h3>
                <p className="text-[11px] text-slate-500 font-bold mb-6">Extração de Calcário - Paraná/PR</p>
                <button className="w-full bg-slate-800 p-3 rounded font-black text-[9px] hover:bg-slate-700 uppercase tracking-widest transition-colors">Ver Histórico de Jazidas</button>
              </div>
              <div className="bg-[#161b22] p-8 border border-[#30363d] rounded-lg border-dashed flex items-center justify-center">
                <p className="text-slate-600 text-[10px] font-black uppercase tracking-widest italic">+ NOVO CLIENTE FIXO</p>
              </div>
            </div>
          </div>
        )}
      ...
      {isFirstLogin && (
        <div className="fixed inset-0 bg-black flex items-center justify-center p-4 z-[100] backdrop-blur-md">
          <div className="bg-[#161b22] p-10 rounded border border-emerald-500/30 w-full max-w-md shadow-2xl text-center">
            ...
            <button onClick={() => {}} className="w-full mt-10 bg-emerald-600 hover:bg-emerald-500 p-5 rounded font-black uppercase text-xs tracking-widest shadow-lg transition-all">Sincronizar Acesso</button>
          </div>
        </div>
      )}
    </div>
